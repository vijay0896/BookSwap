...
exports.getAllUsers = async (req, res) => {
  try {
    ...
    const formatted = results.map((user) => formatUserData(user));
    res.json(formatted);
  } catch (error) {
...
exports.updateUserDetails = async (req, res) => {
  ...
  // With Cloudinary, you'll get the secure_url from the upload response
  // This assumes your upload middleware stores the Cloudinary URL in req.file.secure_url
  let profileImageUrl = req.file?.secure_url || null;
